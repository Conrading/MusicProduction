...
//local server that handles the different types of messages 
const MESSAGE_TYPE = {
  chain: "CHAIN",
  block: "BLOCK",
...
class P2pserver {
  ...
  //handling the message according to the message type
    ...
    socket.on("message", message => {
      ...
      switch (data.type) {
        ...
        case MESSAGE_TYPE.transaction:
          if (!this.transactionPool.transactionExists(data.transaction) && Transaction.transactionContentVerify(data.transaction, this.blockchain)) {
  ...
  ///////////  for testing of broken node  ////////////////
  broadcastChain(chain) {
    this.sockets.forEach(socket => {
      this.sendBrokenChain(socket, chain);
    });
  }

  sendBrokenChain(socket, chain) {
    socket.send(
      JSON.stringify({
        type: MESSAGE_TYPE.chain,
        chain: chain
